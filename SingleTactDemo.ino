...
void setup()
{
   ...
   pinMode(led, OUTPUT);   
   //Serial.begin(115200*4);  // start serial for output
...
boolean ProcessIncomingSerialData()
{
   ...
   if(Serial.available() > 0) 
      serialIncomingBuffer[serialIncomingBufferIndex_] = Serial.read(); //Read the new byte
   else
   ...
   if(serialIncomingBufferIndex_ >MINIMUM_FROMPC_PACKETLENGTH) 
   {
      ...
      {     
         ...
         {
            return true; //We have a good packet
         }
         else
         {
            ResetSerialBuffer();  //Corrupt packet, reset
            return false;
         } 
      }
   }
